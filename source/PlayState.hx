...
class PlayState extends MusicBeatState
{
	...
	var bgTim1:BGSprite;
	var bgTim2:FlxBackdrop;

	var stageFrontTimmy1:BGSprite;
	var stageFrontTimmy2:FlxBackdrop;

	var slidingOponent:Bool = false;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'gurtStage': //Week 1
				var bg:BGSprite = new BGSprite('Gurt_BG', -600, -200, 0.9, 0.9);
				add(bg);

				bg2Overlay = new BGSprite('Gurt_Overlay', -600, -200, 0.9, 1);
				

			
			case 'moaiHouse': //Week 1
				dadbattleSmokes = new FlxSpriteGroup(); //troll'd

			
			case 'stageCorrupted': //Week dad-battle-2
				var bg:BGSprite = new BGSprite('stageback', -200, -150, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('stagefront', -150, 350, 0.9, 0.9);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
				add(stageFront);
				if(!ClientPrefs.lowQuality) {
					var stageLight:BGSprite = new BGSprite('stage_light', -5, -20, 0.9, 0.9);
					stageLight.setGraphicSize(Std.int(stageLight.width * 1.1));
					stageLight.updateHitbox();
					add(stageLight);
					var stageLight:BGSprite = new BGSprite('stage_light', 225, -50, 0.9, 0.9);
					stageLight.setGraphicSize(Std.int(stageLight.width * 1.1));
					stageLight.updateHitbox();
					stageLight.flipX = true;
					add(stageLight);

					var stageCurtains:BGSprite = new BGSprite('stagecurtains', -125, -150, 1.3, 1.3);
					stageCurtains.setGraphicSize(Std.int(stageCurtains.width * 0.9));
					stageCurtains.updateHitbox();
					add(stageCurtains);
				}
			...
			case 'DaVoid': //Week void
				var bg:BGSprite = new BGSprite('stagebackVoid', -600, -200, 0.9, 0.9);
				add(bg);

			case 'blocco': //Week blocco
				GameOverSubstate.deathSoundName = 'fnf_loss_sfx-shot';
				GameOverSubstate.loopSoundName = 'gameOver-shot';
				GameOverSubstate.endSoundName = 'gameOverEnd-shot';
				GameOverSubstate.characterName = 'bf-dead-shot';

				var bg:BGSprite = new BGSprite('stagebackBlocco', -600, -200, 0.9, 0.9);
				add(bg);


			case 'schoolTimmy': //Week timmy
				GameOverSubstate.characterName = 'bf-dead-dither';

				slidingOponent = false;

				bgTim1 = new BGSprite('stagebackTimmySchool', -600, -212, 0.9, 0.9);
				add(bgTim1);

				precacheList.set('Koneko1', 'image');

				precacheList.set('Koneko2', 'image');

				precacheList.set('Koneko3', 'image');

				precacheList.set('Koneko4', 'image');

				bgTim2 = new FlxBackdrop(Paths.image('stagebackTimmySchool'), 0.9, 0, true, true);
				bgTim2.x = -600;
				bgTim2.y = -212;
				bgTim2.velocity.set(1000, 0);
				bgTim2.visible = false;
				add(bgTim2);

				stageFrontTimmy1 = new BGSprite('stagefrontTimmySchool', -650, 600, 0.9, 0.9);
				stageFrontTimmy1.setGraphicSize(Std.int(stageFrontTimmy1.width * 1.1));
				stageFrontTimmy1.updateHitbox();
				add(stageFrontTimmy1);

				stageFrontTimmy2 = new FlxBackdrop(Paths.image('stagebackTimmyStageFrontScroll'), 0.9, 0);
				stageFrontTimmy2.x = -600;
				stageFrontTimmy2.y = -212;
				stageFrontTimmy2.velocity.set(1000, 0);
				stageFrontTimmy2.visible = false;
				add(stageFrontTimmy2);
		...
		switch(charSelection){
			...
			case 1:
			boyfriend = new Boyfriend(0, 0, 'bf-2');
			case 2:
			boyfriend = new Boyfriend(0, 0, 'bf-tim');
			default:
				boyfriend = new Boyfriend(0, 0, SONG.player1);
			}
  ...
  public function clearShaderFromCamera(cam:String){
	...
	function doKonekoFunny(konekoInt:Int = 0, fuck:Bool = false)
	{
		trace('koneko' + konekoInt);
		var daKoneko:FlxSprite = new FlxSprite(0,0);
		// CachedFrames.cachedInstance.get('koneko')


		switch(konekoInt)
		{
			case 0:
				daKoneko.frames = Paths.getSparrowAtlas('Koneko1');
				daKoneko.animation.addByPrefix('konekoAnim','Koneko1',24, false);
				daKoneko.y = -170;
				daKoneko.setGraphicSize(Std.int(daKoneko.width * 1.25));
			case 1:
				daKoneko.frames = Paths.getSparrowAtlas('Koneko2');
				daKoneko.animation.addByPrefix('konekoAnim','Koneko2',24, false);
				daKoneko.y = 165;
				daKoneko.setGraphicSize(Std.int(daKoneko.width * 1.5));
			case 2:
				daKoneko.frames = Paths.getSparrowAtlas('Koneko3');
				daKoneko.animation.addByPrefix('konekoAnim','Koneko3',24, false);
				daKoneko.y = 145;
			case 3:
				daKoneko.frames = Paths.getSparrowAtlas('Koneko4');
				daKoneko.x = 165;
				daKoneko.animation.addByPrefix('konekoAnim','Koneko4',24, false);
		}

		daKoneko.setGraphicSize(Std.int(daKoneko.width * 1.25));

		daKoneko.cameras = [camHUD];

		add(daKoneko);
		daKoneko.flipX = fuck;
		daKoneko.animation.play('konekoAnim');
		daKoneko.animation.finishCallback = function(pog:String)
			{
				trace('ended koneko');
				remove(daKoneko);
			}
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			...
			case 'DaVoid':
				var dadFly_Y = 270 + -Math.sin((curStep / 9.5) * 2) * 1000 * 0.45;
				var dadFly_X = -30 -Math.cos(curStep / 9.5) * 200;

				dad.x += (dadFly_X - dad.x) / 12;
				dad.y += (dadFly_Y - dad.y) / 12;
			...
			case 'schoolTimmy':
				...
				if(slidingOponent)
					{
					dad.x += (dadFly_X - dad.x) / 16;
					}

			case 'stageCorrupted':
				var dadFly_Y = 270 + -Math.sin((curStep / 9.5) * 2) * 5000 * 0.45;
				var dadFly_X = -30 -Math.cos(curStep / 9.5) * 300;

				dad.x += (dadFly_X - dad.x) / 12;
				dad.y += (dadFly_Y - dad.y) / 12;

				var bfFly_Y = 0 + -Math.sin((curStep / 9.5) * 2) * 1500 * 0.45;
				var bfFly_X = 550 -Math.cos(curStep / 9.5) * 100;

				boyfriend.x += (bfFly_X - boyfriend.x) / 6;
				boyfriend.y += (bfFly_Y - boyfriend.y) / 6;

				var gfFly_Y = 0 + -Math.sin((curStep / 9.5) * 2) * 100 * 0.45;
				var gfFly_X = 0 -Math.cos(curStep / 9.5) * 8000;

				gf.x += (gfFly_X - gf.x) / 3;
				gf.y += (gfFly_Y - gf.y) / 3;

			case 'stinkyPoopShit':
				var dadFly_Y = 270 + -Math.sin((curStep / 9.5) * 2) * 500 * 0.45;
				var dadFly_X = -30 -Math.cos(curStep / 9.5) * 500;
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) { 
		switch(eventName) {
			...
			case 'Trigger ScrollingBG':

						bgTim2.visible = true;

						bgTim1.visible = false;

						stageFrontTimmy2.visible = true;

						stageFrontTimmy1.visible = false;
				case 'Disable ScrollingBG':
			...
			case 'Change Health Drain':
				var val1:Float = Std.parseFloat(value1);
				var val2:Float = Std.parseFloat(value2);

				SONG.healthdrain = val1;


			case 'Toggle Health Drain Can Kill':
				var val1:Float = Std.parseFloat(value1);
				var val2:Float = Std.parseFloat(value2);
				...
				switch(charType) {
					case 1:
						SONG.healthdrainKill = true;
					case 2:
						SONG.healthdrainKill = false;

				}
			case 'Deduct Health Drain':
				var val1:Float = Std.parseFloat(value1);
				var val2:Float = Std.parseFloat(value2);

				SONG.healthdrain = (SONG.healthdrain - val1);

			case 'Add Health Drain':
				var val1:Float = Std.parseFloat(value1);
				var val2:Float = Std.parseFloat(value2);
			...
			case 'Open Text File':
				...
				Sys.command('start ' + Paths.txt(value1));

			case 'KonekoDistract':

				var random:Int = FlxG.random.int(0, 3);

				var random2:Int = FlxG.random.int(1, 2);
	...
	override function beatHit()
	{
		...
		if (ClientPrefs.iconbops == "OS") {
			if (dancingLeft){
				iconP1.angle = 12; iconP2.angle = 12; // maybe i should do it with tweens, but i'm lazy // i'll make it in -1.0.0, i promise
			} else { 
				iconP1.angle = -12; iconP2.angle = -12;
			}
		}
