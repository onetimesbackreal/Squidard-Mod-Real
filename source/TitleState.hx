...
class TitleState extends MusicBeatState
{
	...
	var easterEggKeys:Array<String> = [
		'SHADOW', 'RIVER', 'SHUBS', 'BBPANZU', 'BLOCCO', 'GURT'
	];
	...
	function startIntro()
	{
		...
		switch(easterEgg.toUpperCase())
		{
			...
			case 'SHADOW':
				...
				bg.makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
			case 'BLOCCO':
				gfDance.frames = Paths.getSparrowAtlas('bloccoMenu/gfDanceTitle');
				gfDance.animation.addByIndices('danceLeft', 'gfDance', [0, 0], "", 24, false);
				gfDance.animation.addByIndices('danceRight', 'gfDance', [1, 1], "", 24, false);
				logoBl.frames = Paths.getSparrowAtlas('bloccoMenu/logoBumpin');
				bg.loadGraphic(Paths.image('bloccoMenu/titlebg'));
			case 'GURT':
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 16:
					addMoreText('Vs. Squidard'); // credTextShit.text += '\nFunkin';
				case 17:
					skipIntro();
			}
		}
			...
			if (playJingle) //Ignore deez
			{
				...
				switch(easteregg)
				{
					...
					case 'BLOCCO':
						sound = FlxG.sound.play(Paths.sound('JingleShubs'));
					case 'GURT':
						sound = FlxG.sound.play(Paths.sound('JingleGurt'));

						var poop:String = Highscore.formatSong('gurt', 1);

						trace(poop);

						FlxG.save.data.gurtUnlocked = true;
						
						PlayState.SONG = Song.loadFromJson(poop, 'gurt');
					...
					default: //Go back to normal ugly ass boring GF
						sound = FlxG.sound.play(Paths.sound('JingleGurtReverse'));
						remove(ngSpr);
						remove(credGroup);
